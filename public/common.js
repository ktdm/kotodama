...
function stringedit(it,start) {
 if (start) {
  ...
  (i=it.previousSibling).innerHTML=it.value;
  it.value=""
 }
...
function ins() {
 var x,z,n,c,l=getTarget(this); 
 if (typeof(arguments[0])!=="string"&&!$("ins_item")) {
  list=lists[l];
  list.actions=new List(l);
  x=$(l).appendChild($("ins_stamp").cloneNode(true));
  x.id="ins_item";
  (inp=x.childNodes[1]).id="ins";
  inp.value=lists[l].actions.typelabel;
  inp.onfocus=function(){inputFocus("ins",lists[l].actions.typelabel)};
  inp.onblur=function(){inputBlur("ins",lists[l].actions.typelabel)};
  x.style.display="block";
 }
 else {
  $("ins_item").parentNode.removeChild($("ins_item"));
